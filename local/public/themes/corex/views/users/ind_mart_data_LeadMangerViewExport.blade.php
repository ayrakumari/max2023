...
<div class="m-content">
	...
	<div class="m-portlet m-portlet--mobile">
		...
		<div class="m-portlet__body">
			...
			<!-- form  -->
			<!--begin::Section-->
			<div class="m-section" style="display:none">
			...
			<!--begin: Datatable -->
			<table class="table table-striped- table-bordered table-hover table-checkable ajayra" id="m_table_LEADList_LeadMangerVIEWExport">
				<thead>
					<tr>
...
<div class="modal fade" id="m_modal_LeadAssignModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Lead Assignment Export</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
				...
				<form>
					...
					<div class="form-group">
						...
						<select class="form-control m-input" id="assign_user_id">
							...
							if(Auth::user()->id==76){
								?>
									<option value="76">Deepansu Konjolia</option>
							...
							$user_role = $userRoles[0];
							?>
							@if ($user_role =="Admin" || Auth::user()->id==3 || Auth::user()->id==40 || Auth::user()->id==4 || Auth::user()->id==119 || Auth::user()->id==129 || Auth::user()->id==90 || Auth::user()->id==171 || Auth::user()->id==130 || Auth::user()->id==131 || Auth::user()->id==134 || Auth::user()->id==144 || Auth::user()->id==145 || Auth::user()->id==141 || Auth::user()->id==138)
							@foreach (AyraHelp::getSalesAgentAdmin() as $user)
							@if ($user->id==134 ||  $user->id==135 || $user->id==136 || $user->id==134 || $user->id==144 || $user->id==145 || $user->id==141 || $user->id==138 
							|| $user->id==78
							|| $user->id==83
							|| $user->id==85
							|| $user->id==84
							...
							@endif
							<!-- <option value="102">DEEPIKA JOSHI</option> -->
							<!-- <option value="119">Manish Sain</option> -->
							<option value="143">Jatin Chopra</option>
							<option value="178">Sushant Gupta</option>
								<?php
							}
							?>

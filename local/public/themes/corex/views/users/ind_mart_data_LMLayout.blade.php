    ...
    <div class="m-portlet m-portlet--mobile">
       <div class="m-portlet__head">
          ...
          <div class="m-portlet__head-tools">
			 ...
			 <?php
			 if($user_role=='Admin' || $user_role=='SalesHead' || Auth::user()->id==217 ||  Auth::user()->id==202){
				 ?>
       ...
       <a href="javascript::void(0)" title="Click here to refresh Lead List"  onclick="viewFreshLead()" class="btn btn-warning btn-sm m-btn 	m-btn m-btn--icon">
														...
														<a href="javascript::void(0)" onclick="viewHOLDLead_LM()" class="btn btn-warning btn-sm m-btn 	m-btn m-btn--icon">
															<span>
														...
														<a href="javascript::void(0)" onclick="viewAllIreevant_LM()" class="btn btn-secondary btn-sm m-btn 	m-btn m-btn--icon">
															<span>
																<i class="fa flaticon-list-3"></i>
																<span>Irrelevant</span>
															</span>
														</a>
														<a href="javascript::void(0)" onclick="viewDUPLICATELead_LM()" class="btn btn-warning btn-sm m-btn 	m-btn m-btn--icon">
															<span>
															<i class="fa fa-hand-point-right"></i>
																<span>DUPLICATE</span>
															</span>
								...
								<form class="m-form m-form--fit m--margin-bottom-20">
									<div class="row m--margin-bottom-20">
										<div class="col-lg-3 m--margin-bottom-10-tablet-and-mobile">
											<label>Company:</label>
											<input type="text" class="form-control m-input" placeholder="E.g: 4590" data-col-index="1">
										</div>
                                        <div class="col-lg-3 m--margin-bottom-10-tablet-and-mobile">
											<label>Mobile:</label>
											<input type="text" class="form-control m-input" placeholder="E.g: 4590" data-col-index="2">
										</div>
		  ...
		  <div class="m-section">
											...
											<div class="m-section__content">
												<div class="m-demo" data-code-preview="true" data-code-html="true" data-code-js="false">
														...
														<a href="javascript::void(0)" onclick="viewDIRECTLead_LM()" class="btn btn-outline-primary btn-sm 	m-btn m-btn--icon">
															<span>
																<i class="fa flaticon-list-2"></i>
																<span>DIRECT</span>
															</span>
														</a>
														<a href="javascript::void(0)" onclick="viewBUYLead_LM()" class="btn btn-outline-accent btn-sm 	m-btn m-btn--icon">
															<span>
																<i class="fa flaticon-multimedia-2"></i>
																<span>BUY</span>
															</span>
														</a>
														<a href="javascript::void(0)" onclick="viewPHONELead_LM()" class="btn btn-outline-success btn-sm 	m-btn m-btn--icon">
															<span>
															<i class="flaticon-support"></i>
																<span>PHONE</span>
															</span>
														</a>
														<a href="javascript::void(0)" onclick="viewINHOUSELead_LM()" class="btn btn-outline-warning 	btn-sm 	m-btn m-btn--icon">
															<span>
...
<!--begin: Datatable -->
			<table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_LEADList_LMLAYOUT">
				<thead>
					<tr>
...
<div class="modal fade" id="m_modal_LeadAssignModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog modal-sm" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel"> A Lead Assignment</h5>
										<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span aria-hidden="true">&times;</span>
										</button>
									</div>
										...
										<form>
										   <input type="hidden" id="QUERY_ID" >
											<div class="form-group">
                                                ...
                                                $user_role = $userRoles[0];
                                                ?>
                                                @if ($user_role =="Admin" || Auth::user()->id==3   || Auth::user()->id==77 || Auth::user()->id==90 || Auth::user()->id==171 || Auth::user()->id==134  ||  Auth::user()->id==144  ||  Auth::user()->id==141   || Auth::user()->id==139 ||  Auth::user()->id==135 || Auth::user()->id==136 || Auth::user()->id==221)
                                                @foreach (AyraHelp::getSalesAgentAdmin() as $user)
												@if ($user->id==134 || $user->id==135 ||  $user->id==136 || $user->id==139  || $user->id==141  ||   $user->id==144
												|| $user->id==78
												|| $user->id==83
												|| $user->id==85
												|| $user->id==84
						 ...
						 <div class="modal-body">
							 ...
							 <div class="form-group">
									...
									<select class="form-control m-input"  id="assign_user_id_calldata">
																					...
																					$user_role = $userRoles[0];
																					?>
																					@if ($user_role =="Admin" || Auth::user()->id==3  || Auth::user()->id==139 || Auth::user()->id==134 ||  Auth::user()->id==141  ||  Auth::user()->id==142   ||  Auth::user()->id==140 ||  Auth::user()->id==84|| Auth::user()->id==90 || Auth::user()->id==171 || Auth::user()->id==130 || Auth::user()->id==131)
																					@foreach (AyraHelp::getSalesAgentAdmin() as $user)
									@if ($user->id==134 || $user->id==135 ||  $user->id==136 || $user->id==139  || $user->id==141 
									|| $user->id==78
									|| $user->id==83
									|| $user->id==85
									|| $user->id==84

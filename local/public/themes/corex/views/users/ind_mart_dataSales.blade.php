...
<div class="m-content">
	...
	<div class="m-portlet m-portlet--mobile">
			...
			<div class="m-portlet__head-caption">
				<div class="m-portlet__head-title">
					<h3 class="m-portlet__head-text">
						...
						if ($user_role == 'Admin' || $user_role == 'SalesHead' || Auth::user()->id == 77) {
						?>
		...
		<div class="m-portlet__body">
					...
					<div class="row m--margin-bottom-20">
						...
						<div class="col-lg-3 m--margin-bottom-10-tablet-and-mobile">
							...
							<select class="form-control m-input" data-col-index="7">
								...
								/*$lead_arr = DB::table('indmt_data')->distinct()->get(['COUNTRY_ISO']);


								foreach ($lead_arr as $key => $rowData) {

									$countryISO = DB::table('countries')->where('iso_code_2', $rowData->COUNTRY_ISO)->first();
									if ($countryISO != null) {
								?>
										<option value="{{$rowData->COUNTRY_ISO}}">{{$countryISO->name}}</option>
									<?php
									} else {
									?>
										<option value="{{$rowData->COUNTRY_ISO}}">{{$rowData->COUNTRY_ISO}}</option>
									<?php
									}
			...
			<!-- form  -->
			<!--begin: Datatable -->
			<table class="table table-striped- table-bordered table-hover table-checkable ajayra" id="m_table_LEADListSALES_END">
				<thead>
					<tr>
...
<div class="modal fade" id="m_modal_LeadUnQliFiedModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Lead Action</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form>
					...
					<div class="form-group">
						<label for="message-text" class="form-control-label">Action Type:</label>
						<select required name="unqlified_type" id="unqlified_type" class="form-control">
							<!-- <option value="">--SELECT--</option> -->
							<?php
							 $user = auth()->user();
							 $userRoles = $user->getRoleNames();
...
<!-- m_modal_6 -->
<div class="modal fade" id="m_modal_noResponseModel_sales" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLongTitle">Lead No Response</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<input type="hidden" id="QUERY_IDB_salesNOResp">
				<div class="form-group">
					<label for="message-text" class="form-control-label">*Comment :</label>
					<textarea class="form-control" id="txtNoRespLead" name="txtNoRespLead"></textarea>
				</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
				<button type="button" id="btnLeadNoRespSales" class="btn btn-primary">Save</button>
			</div>
		</div>
	</div>
</div>

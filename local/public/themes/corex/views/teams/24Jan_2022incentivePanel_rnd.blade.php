        ...
        <div class="m-portlet">
            ...
            <div class="m-portlet__body">
                <ul class="nav nav-tabs  m-tabs-line m-tabs-line--success" role="tablist">
                    ...
                    <li class="nav-item dropdown m-tabs__item" style="display: none;">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="m_tabs_inc_1" role="tabpanel">
                        ...
                        <div class="m-accordion m-accordion--default m-accordion--solid" id="m_accordion_3" role="tablist">
                            <?php
                            $lead_data = AyraHelp::getAllWorkingUserIncentiveRND();
                            $i = 0;
                            ...
                            ?>
                                ...
                                <div class="m-accordion__item">
                                    <div class="m-accordion__item-head collapsed" role="tab" id="m_accordion_3_item_{{$row['uid']}}_head" data-toggle="collapse" href="#m_accordion_3_item_{{$row['uid']}}_body" aria-expanded="    false">
                                        ...
                                        <span class="m-accordion__item-title">
                                            ...
                                            <a title="View All " href="javascript::void(0)" class="btn btn-secondary m-btn btn-sm 	m-btn m-btn--icon">
                                                <span>
                                                    <b>Formulated</b>:
                                                    <span class="m-badge m-badge--primary" title="From {{date('j F Y',strtotime($quarter_start_date))}} To {{date('j F Y',strtotime($quarter_end_date))}}"> {{AyraHelp::getFormulatedBetweenDate($row['uid'],$quarter_start_date,$quarter_end_date)}}</span>
                                            ...
                                            <a title="View All " href="javascript::void(0)" class="btn btn-secondary m-btn btn-sm 	m-btn m-btn--icon">
                                                <span>
                                                    <b>Production</b>:
                                                    <?php
                                                    $dataPr = AyraHelp::getSampleProductiondBetweenDate($row['uid'], $quarter_start_date, $quarter_end_date);
                                                    // print_r($dataPr);



                                                    ?>
                                                    <span class="m-badge m-badge--success" title="From {{date('j F Y',strtotime($quarter_start_date))}} To {{date('j F Y',strtotime($quarter_end_date))}}"> {{$dataPr['count']}}</span>
                                            ...
                                            <div class="m-portlet">
                                                ...
                                                <div style="background-color: #f4f6f7;" class="m-portlet__body">
                                                    <ul class="nav nav-tabs nav-fill" role="tablist">
                                                        <li class="nav-item">
                                                            <a class="nav-link active" data-toggle="tab" href="#m_tabs_2_1_C_{{$row['uid']}}">Current Month</a>
                                                        </li>
                                                        <li class="nav-item">
                                                            <a class="nav-link" data-toggle="tab" href="#m_tabs_2_2_H_{{$row['uid']}}">History</a>
                                                        </li>


                                                    </ul>
                                                    <div class="tab-content">
                                                        <div class="tab-pane active" id="m_tabs_2_1_C_{{$row['uid']}}" role="tabpanel">
                                                            <!-- Currening  -->
                                                            <!--begin::m-widget5-->
                                                            <!-- <b>FROM :1 AUG 2021 to 31 AUG 2021 Data : TEST</b> -->
                                                            <div class="m-widget5">
								...
								<tbody>
									...
									$qc_formsArrData = DB::table('qc_forms')
										...
										->where('order_type', '=', 'Private Label')
										//  ->where('chemist_id', '=', $user_id)
										->get();
									
									// echo "<pre>";
									// print_r($qc_formsArrData);
									// die;
									...
									foreach ($qc_formsArrData as $key => $rowData) {


										$qc_DispatchedData = DB::table('order_dispatch_data')
											->where('form_id', $rowData->form_id)
											->latest()
											->first();
										if ($qc_DispatchedData != null) {
											$disDate = date('j-M-y',strtotime($qc_DispatchedData->dispatch_on)) . '(D)';
										} else {
                                            if(isset($rowData->curr_stage_updated_on)){
                                                $disDate = '(YD)';

                                            }else{
                                                $disDate = date('j-M-y ',strtotime($rowData->curr_stage_updated_on)) . '(YD)';
                                            }
										...
										$clientId = optional($client_arr)->company . " | " . optional($client_arr)->brand;
										 ...
										 $sidcode = $rowData->item_fm_sample_no;
										//$sidcode='POJ-5541-2';
										$sampleItemArr = DB::table('sample_items')
											->where('sid_partby_code', 'LIKE', "%{$rowData->item_fm_sample_no}")
											->first();
											$brandStr = "";
										if ($sampleItemArr == null) {
											...
											// die;
											// echo $sampleItemArr->sid;
											// echo "<br>";


											$sampleArr = DB::table('samples')
												->where('id', $sampleItemArr->sid)
												->whereNotNull('brand_type')
												->first();
											//print_r($sampleArr->brand_type);
											// die;

											$amt = 0;
											if ($rowData->order_repeat == 2) {
												echo $smType = $sampleArr->brand_type;
												//$smType = 2;
										...
										// $orderRepeatCount = DB::table('qc_forms')
										// ->where('item_fm_sample_no', 'LIKE', "%{$rowData->item_fm_sample_no}%")                                                                                         
										// ->count();
									...
									?>
										<tr>
											...
											<td>{{$clientId}}</td>
											<td>
												{{$rowData->order_id}}/{{$rowData->subOrder}}
											</td>
											<td>{{@$rowData->item_name}} {{@$rowData->chemist_id}}</td>
											<td>{{$disDate}}</td>
									...
									<tr>
										...
										<td></td>
                                                        ...
                                                        <div class="tab-pane" id="m_tabs_2_2_H_{{$row['uid']}}" role="tabpanel">
                                                            <!-- History -->
                                                            <!--begin::m-widget5-->
                                                            ...
                                                            <div class="m-widget5">
                                                                <div class="m-widget5__item">
                                                                        ...
                                                                        <table class="table table-bordered m-table m-table--border-success">
                                                                            ...
                                                                            <tbody>
                                                                                <tr>
                                                                                    ...
                                                                                    $dataArrDuration = DB::table('incentive_rnd_history')->get();
                                                                                    foreach ($dataArrDuration as $key => $rowData) {
                                                                                        $durationIncentive = $rowData->name . " [" . date('j F Y', strtotime($rowData->start_date)) . " TO " . date('j F Y', strtotime($rowData->end_date)) . "]";
                                                                                        ?>
                                                                                <tr>
                                                                                    <td>{{$rowData->id}}</td>
                                                                                    ...
                                                                                    <td>
                                                                                        ...
                                                                                        <a target="_blank" title="View Details" class="btn btn-warning m-btn m-btn--icon btn-sm m-btn--icon-only  m-btn--pill m-btn--air" href="{{route('viewIncentiveEligibilityPanel_History_RND',['uid'=>$row['uid'],$rowData->in_month,1,$rowData->in_year])}}">
                                                                    ...
                                                                    <!-- <th scope="row">1</th>
                                                                    <td>Incentive New Sales</td>
                                                                    <td>Monthly(1-Nov -30 Nov)</td>
                                                                    <td>---</td>
                                                                    <td>Pending</td>
                                                                    <td> <a href="{{route('viewIncentiveEligibility',$row['uid'])}}" title="View Details" class="btn btn-warning m-btn m-btn--icon btn-sm m-btn--icon-only  m-btn--pill m-btn--air">
                                                                            <i class="fa flaticon-eye"></i>
                                                                        </a>
                                                                    </td> -->
                                                                    </tr>
                                                            ...
                                                            <div class="m-widget5__info">
                                                                ...
                                                                <span class="m-widget5__info-date m--font-info">
                                                                    <a href="{{route('viewIncentiveEligibility',$row['uid'])}}" title="View Details" class="btn btn-warning m-btn m-btn--icon btn-sm m-btn--icon-only  m-btn--pill m-btn--air">
                                                                        <i class="fa flaticon-eye"></i>
                                                                    </a>

    ...
    <div class="m-portlet m-portlet--mobile" style="display:#">
            ...
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text">
                        ...
                        // echo "<pre>";
                        // print_r($data);
                        $boFormulaChildArr = DB::table('bo_formuation_child_v1')
                            ->where('fm_id', $data->id)
                            ->get();
                            $boFormulaChildArrUniq = DB::table('bo_formuation_child_v1')
                            ->where('fm_id', $data->id)    
                            ->orderBy('phase', 'asc')          
                            ->distinct()              
                            ->get('phase');
                        $boFormulaChildProcessArr = DB::table('bo_formuation_child_process_v1')
                            ->where('fm_id', $data->id)
                            ->get();

                        $boFormulaChildArrSum = DB::table('bo_formuation_child_v1')
                            ->where('fm_id', $data->id)
                            ->sum('mfg_pecentage');
        ...
        <div class="m-portlet__body">
           ...
           foreach ($boFormulaChildArrUniq as $key => $rowData) {
            ...
            $boFormulaChildProcessArrChild = DB::table('bo_formuation_child_process_v1')
            ->where('fm_id', $data->id)

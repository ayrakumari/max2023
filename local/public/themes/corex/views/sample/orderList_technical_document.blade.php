	...
	<div class="m-portlet m-portlet--mobile">
			...
			<div class="m-portlet__head-tools">
				<ul class="m-portlet__nav">
					<li class="m-portlet__nav-item">
						@can('add-samples')
						<a href="{{route('sampleAddTechinalDocumentOrder')}}" class="btn btn-primary m-btn m-btn--custom m-btn--icon">
							<span>
		...
		<div class="m-portlet__body">
			...
			<div class="m-form__group form-group">
				<label for="">Choose Any Option</label>
				<div class="m-radio-inline">
					<label class="m-radio">
						<input type="radio" checked name="choose_status" value="1"> <b>Pending</b>
						<span></span>
					</label>
					<label class="m-radio">
						<input type="radio" name="choose_status" value="2"> <b>Completed</b>
						<span></span>
					</label>
					<label class="m-radio">
						<input type="radio" name="choose_status" value="3"> <b>show All</b>
						<span></span>
					</label>
				</div>
			...
			<!--begin: Datatable -->
			<table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_OrderList_technicaldoc">
				<thead>
					<tr>
						<th>ID#</th>
						<th>Order Date </th>
...
<!--begin::Modal-->
<div class="modal fade" id="m_select2_modal_sampleTech" role="dialog" aria-labelledby="" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="">Link Existing Document</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true" class="la la-remove"></span>
				</button>
			</div>
			<form method="post" action="{{route('saveSampleTechLinkingData')}}" class="m-form m-form--fit m-form--label-align-right" id="m_form_3_sampleTechSubmitFeedbackLink">
				@csrf
				<input type="hidden" name="txtSampleID" id="txtSampleID">
				<div class="modal-body">
					<div class="form-group m-form__group row m--margin-top-20">
						<label class="col-form-label col-lg-3 col-sm-12">Search Item</label>
						<div class="col-lg-9 col-md-9 col-sm-12">
							<?php
							$samples_techArr = DB::table('samples_tech_list')
								// ->select('item_name', 'id', 'item_requirement','sample_id')
								->whereNotNull('doc_name')
								->distinct()
								->get('sample_id');

							?>
							<select name="tech_item_name" class="form-control m-bootstrap-select m_selectpicker" data-live-search="true">

								@foreach ($samples_techArr as $rowdata)
								<?php
								$samples_techArrChildArr = DB::table('samples_tech_list')
									->select('item_name', 'id', 'item_requirement')
									->whereNotNull('doc_name')
									->where('sample_id', $rowdata->sample_id)
									->get();
								$strReq = "";
								$itemName = "";
								foreach ($samples_techArrChildArr as $key => $data) {
									$itemName = $data->item_name;
									switch ($data->item_requirement) {
										case 1;
											$strReq .= "Ingredent,";
											break;
										case 2;
								...
								?>
								<option value="{{$rowdata->sample_id}}">{{$itemName}} {{$strReq}}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group m-form__group row m--margin-top-20">
						<label class="col-form-label col-lg-3 col-sm-12">Sample ID</label>
						<div class="col-lg-9 col-md-9 col-sm-12">
							<?php
							$samples_techArr = DB::table('samples')
								// ->select('item_name', 'id', 'item_requirement','sample_id')
								->where('status', 2)
								->distinct()
								->get();
							...
							<select name="select_sample_id_ori" class="form-control m-bootstrap-select m_selectpicker showSampleItemTechDataSelectUI" data-live-search="true">
					...
					<div class="form-group m-form__group row m--margin-top-20">
						<div class="col-lg-9 col-md-9 col-sm-12 showSampleItemTechDataUI">
					...
					<div class="form-group m-form__group row m--margin-top-20">
						...
						<div class="col-lg-9 col-md-9 col-sm-12">
							<textarea class="form-control m-input" name="tech_notes" id="exampleTextarea" rows="2"></textarea>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="submit" data-wizard-action="submitSaveSampleTechLink" class="btn btn-primary submitSaveSampleTech">Submit</button>
				</div>
			</form>
		</div>
	</div>
...
<!--begin::Modal-->
<div class="modal fade" id="m_select2_modal_sampleTechDOC" role="dialog" aria-labelledby="" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="">Upload Document</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true" class="la la-remove"></span>
				</button>
			</div>
			<form method="post" action="{{route('saveOrderTechDocFeedback_DOC')}}" enctype="multipart/form-data" class="m-form m-form--fit m-form--label-align-right" id="m_form_3_OrderTechSubmitFeedbackDOC">
				@csrf
				<input type="hidden" name="txtSampleID_DOC" id="txtSampleID_DOC">
				<div class="modal-body">
					<div class="form-group m-form__group row m--margin-top-20">
						<label class="col-form-label col-lg-3 col-sm-12">Select Document</label>
						<div class="col-lg-6 col-md-6 col-sm-12">
							<div class="custom-file">
								<input title="PDF file only" accept="application/pdf" type="file" name="customFileTechDoc" class="custom-file-input" id="customFileTechDoc">
								<label class="custom-file-label" for="customFile">Choose file</label>
							</div>
						</div>
						<div class="col-lg-3 col-md-3 col-sm-12">
							<span style="margin-top:9px" class="m-badge m-badge--primary m-badge--wide m-badge--rounded strREQ">in process</span>
						</div>
					</div>
					<div class="form-group m-form__group row m--margin-top-20 ingreInputdata">
						<label class="col-form-label col-lg-3 col-sm-12">Ingredents</label>
						<div class="col-lg-9 col-md-9 col-sm-12">
							<textarea class="form-control m-input" name="tech_ingredents" id="exampleTextarea" rows="2"></textarea>
						</div>
					...
					<div class="form-group m-form__group row m--margin-top-20">
						<label class="col-form-label col-lg-3 col-sm-12">Notes</label>
						<div class="col-lg-9 col-md-9 col-sm-12">
							<textarea class="form-control m-input" name="tech_notes5" id="exampleTextarea" rows="2"></textarea>
						</div>
				...
				<div class="modal-footer">
					...
					<button type="submit" data-wizard-action="submitSaveSampleTech" class="btn btn-primary submitSaveSampleTech">Submit</button>
...
<!--begin::Modal-->
<div class="modal fade" id="m_select2_modal_sampleTechDOC_viewDetail" role="dialog" aria-labelledby="" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="">View Sample Details</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true" class="la la-remove"></span>
				</button>
			</div>
			<div class="modal-body showSampleAllDetails">
				dd
			</div>
		</div>
	</div>

...
<div class="m-content">
	...
	<div class="m-portlet m-portlet--mobile">
		<div class="m-portlet__head">
			<div class="m-portlet__head-caption">
				<div class="m-portlet__head-title">
					<h3 class="m-portlet__head-text">
						...
						if ($user_role == 'Admin' || $user_role == 'SalesHead' || $user_role == 'Intern' || Auth::user()->id == 134 ||  Auth::user()->id == 141  ||  Auth::user()->id == 144 ||  Auth::user()->id == 145 || Auth::user()->id == 3 || Auth::user()->id == 40) {
						?>
			...
			<div class="m-portlet__head-tools">
				<ul class="m-portlet__nav">
					<?php
					if ($user_role == 'Admin' || $user_role == 'SalesHead' || Auth::user()->id == 134 || Auth::user()->id == 141 ||   Auth::user()->id == 130 ||  Auth::user()->id == 131) {
					?>
		...
		<div class="m-portlet__body">
			...
			$userRoles = $user->getRoleNames();
			$user_role = $userRoles[0];
			if ($user_role == 'Admin' || $user_role == 'SalesHead' || Auth::user()->id == 134 || Auth::user()->id == 141 ||   Auth::user()->id == 144 ||   Auth::user()->id == 145 || Auth::user()->id == 3 || Auth::user()->id == 4 || Auth::user()->id == 40) {
			?>
			...
			<div class="m-section" style="display:block">
				...
				<div class="m-section__content">
                    ...
                    <a href="javascript::void(0)" onclick="viewFreshLead()" class="btn btn-outline-primary btn-sm 	m-btn m-btn--icon">
							<span>
								<i class="fa flaticon-list-2"></i>
								<span>ALL LEADS</span>
							</span>
						</a>
						<a href="javascript::void(0)" onclick="viewHOLDLead_V3_Intern()" class="  btn btn-warning btn-sm m-btn 	m-btn m-btn--icon">
						<span>
							<i class="fa fa-hand-point-right"></i>
							<span>HOLD Lead</span>
						</span>
						...
						<!-- <a href="javascript::void(0)" onclick="viewVERIFIED_V2()" class="btn btn-success 	btn-sm 	m-btn m-btn--icon">
							<span>
								<i class="fa flaticon-file"></i>
						...
						<!-- <a href="javascript::void(0)" onclick="viewALL_LEAD_V2()" class="btn btn-primary 	btn-sm 	m-btn m-btn--icon">
							<span>
								<i class="fa flaticon-file"></i>
								<span>ALL FRESH LEAD</span>
							</span>
						</a> -->
						<!-- <a href="javascript::void(0)" onclick="viewALL_LEAD_V2_NONEFRESH()" class="btn btn-primary 	btn-sm 	m-btn m-btn--icon">
							<span>
								<i class="fa flaticon-file"></i>
								<span>NONE FRESH LEAD</span>
							</span>
						</a> -->
			...
			<!--begin: Datatable -->
			<table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_LEADList_LeadMangerVIEW_Intern">
				<thead>
					<tr>
			...
			<div class="modal-body">
				<form>
					...
					<div class="form-group">
						<label for="recipient-name" class="form-control-label">DD Sales Person:</label>
				...
				<form>
					...
					<div class="form-group">
						...
						<select class="form-control m-input" id="assign_user_id">
							...
							$user_role = $userRoles[0];
							?>
							@if ($user_role =="Admin" || Auth::user()->id==3 || Auth::user()->id==40 || Auth::user()->id==4 || Auth::user()->id==119 || Auth::user()->id==129 || Auth::user()->id==90 || Auth::user()->id==171 || Auth::user()->id==171 || Auth::user()->id==130 || Auth::user()->id==131 || Auth::user()->id==134 || Auth::user()->id==144 || Auth::user()->id==145 || Auth::user()->id==141 || Auth::user()->id==221)
							@foreach (AyraHelp::getSalesAgentAdmin() as $user)
							@if ($user->id==134 || $user->id==135 || $user->id==136 || $user->id==134 || $user->id==141  || $user->id->id==221
							|| $user->id==78
							|| $user->id==83
							|| $user->id==85
							|| $user->id==84
...
<!--end::Modal-->
<!-- m_modal_AddDataCallModel -->
<!-- Modal -->
<div class="modal fade" id="m_modal_AddDataCallModel_sales" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLongTitle">Data Call Entry</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<input type="hidden" id="QUERY_IDB_AddDataCallsales">
				<div class="form-group">
					<label for="recipient-name" class="form-control-label">Sales Person:</label>
					<select class="form-control m-input" id="assign_user_id_calldata">
						<option value="">-SELECT-</option>
						...
						$user_role = $userRoles[0];
						?>
						@if ($user_role =="Admin" || Auth::user()->id==3 || Auth::user()->id==77 || Auth::user()->id==90 || Auth::user()->id==130 || Auth::user()->id==131 || Auth::user()->id==171)
						@foreach (AyraHelp::getSalesAgentAdmin() as $user)
						@if ($user->id==134 || $user->id==135 || $user->id==136 || $user->id==144 || $user->id==145
						|| $user->id==78
						|| $user->id==83
						|| $user->id==85
						|| $user->id==84
				...
				<div class="form-group m-form__group">
					...
					<select class="form-control m-input" id="txtDataCallOption">
						<option value="">-SELECT -</option>
						<option value="1">Received Call</option>
						<option value="2">Missed Call </option>
						<option value="3">Call Drop </option>
					</select>
				</div>

				<div class="form-group m-form__group">
					<label for="exampleInputEmail1">Duration(Seconds)</label>
					<input type="number" id="txtDataCallMin" class="form-control m-input" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Sec">
				</div>
				<div class="form-group" style="display:none">
					<label for="message-text" class="form-control-label">*Message:</label>
					<textarea class="form-control" id="txtAddDataCallMessage" name="txtAddDataCallMessage"></textarea>
				</div>
			...
			<div class="modal-footer">
				...
				<button type="button" id="btnAddDataCallSales" class="btn btn-primary">Save</button>
			</div>
		</div>
	</div>
</div>
...
<!-- add tags  -->
<div class="modal fade" id="m_modal_LeadAddTagsModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Lead Tags</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form>
					<input type="hidden" id="QUERY_IDBX">
					...
					foreach ($tagsArr as $key => $rowData) {
						?>
						<label class="m-checkbox">
								<input type="checkbox" name="lead_tag_id" value="{{$rowData->lead_tag_name}}"> {{ $rowData->lead_tag_name }}
								<span></span>
							</label>
						<?php
					}
					...
					<div class="form-group">
						<label for="message-text" class="form-control-label">Message:</label>
						<textarea class="form-control" id="txtMessageTagsReponse"></textarea>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id="btnSubmitLeadTags">Submit Tags Now</button>
			</div>
		</div>
	</div>
</div>

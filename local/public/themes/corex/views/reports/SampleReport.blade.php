...
//$dateToday="2022-08-31";
$tAddedArr = DB::table('samples')
		  ->where('is_deleted', 0)
		  ->whereDate('created_at', $dateToday)
		  ->get();
$sc=0;		  
foreach ($tAddedArr as $key => $rowData) {
	$sc=$sc+count(json_decode($rowData->sample_details));
}		  


$tAdded=$sc;
		  $ssc=0;
		  $sent_onArr = DB::table('samples')
		  ->where('is_deleted', 0)
		  
		  ->whereDate('sent_on', $dateToday)
		  ->get();
		  foreach ($sent_onArr as $key => $rowData) {
			$ssc=$ssc+count(json_decode($rowData->sample_details));
	      }		  
		
		  $sent_on=$ssc;

$tFormulated = DB::table('samples_formula') 

->whereDate('created_on', $dateToday)

->get(); 

$samples30Days = DB::table("samples")
->where('sent_on', '>', now()->subDays(30)->endOfDay())
->count();
$diffD=date('d')-1;

$pendingT=$samples30Days/$diffD;        


DB::table('tbl_samples_report')
	->updateOrInsert(
		['today_date' => $dateToday],
		[         
		'added_count' => $tAdded,
		'formulated_count' => count($tFormulated),
		'dispatched_count' => $sent_on,
		'total_peding' => $pendingT
		]
);
...
?>
      ...
      <table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_SampleReportList">
							<thead>
								<tr>
				...
				<form>
					...
					<div class="form-group">
						...
						<select class="form-control m-input" id="assign_user_id">
							...
							$user_role = $userRoles[0];
							?>
							@if ($user_role =="Admin" || Auth::user()->id==3 || Auth::user()->id==40 || Auth::user()->id==4 || Auth::user()->id==119 || Auth::user()->id==129 || Auth::user()->id==90 || Auth::user()->id==171 || Auth::user()->id==130 || Auth::user()->id==131 || Auth::user()->id==134 || Auth::user()->id==144 || Auth::user()->id==145)
							@foreach (AyraHelp::getSalesAgentAdmin() as $user)
							@if ($user->id==134 || $user->id==135 || $user->id==136 || $user->id==134 || $user->id==144 || $user->id==145
							|| $user->id==78
							|| $user->id==83
							|| $user->id==85
							|| $user->id==84
